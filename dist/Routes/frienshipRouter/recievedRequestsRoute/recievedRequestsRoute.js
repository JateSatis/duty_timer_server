...
const recievedRequestRoute = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        recievedFriendshipRequests = yield initializeConfig_1.DB.getRecievedRequestsByUserId(user.id);
    }
    catch (error) {
    ...
    try {
        recievedFriendshipRequestsInfo = yield initializeConfig_1.DB.getRecievedRequestsInfoByIds(recievedFriendshipRequestsIds);
    }
    catch (error) {
        return res.status(400).json((0, GlobalErrors_1.err)(new GlobalErrors_1.DATABASE_ERROR(error)));
    }
    ...
    try {
        getAllRecievedFriendshipRequestsResponse =
            yield (0, transformRequestsForResponse_1.transformRequestsForResponse)(recievedFriendshipRequestsInfo);
    }
    catch (error) {
